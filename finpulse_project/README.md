...
🔗 **[Live API](https://finpulse-intelligence-c819.onrender.com)** · 🖥️ **[Live Dashboard](https://karishmaram-tech.github.io/finpulse/)** · 📖 **[Architecture](docs/ARCHITECTURE.md)**
...
See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the full request-lifecycle diagram (Client → FastAPI → TTL Cache → Agno Agents → Resilient Tools → External APIs → Postgres).

```
finpulse/
├── agents/team.py          # Agent + Team factory functions
├── config/settings.py      # Environment loading & validation
├── core/
│   ├── app.py               # Execution layer — wires everything into FastAPI
│   ├── metrics.py           # Prometheus middleware + /observability/metrics
│   └── persistence.py       # PostgreSQL run-history persistence
├── models/schemas.py        # Pydantic v2 data contracts
├── tools/finance_tools.py   # Validated, cached, resilient YFinance tools
└── utils/
    ├── cache.py              # Thread-safe in-memory TTL cache
    ├── logging_config.py     # Structured logging setup
    └── resilience.py         # Retry-with-backoff decorator
...
docs/
├── ARCHITECTURE.md            # Full request-lifecycle diagram
└── RESUME_BULLETS.md          # X-Y-Z resume bullet reference
main.py
requirements.txt
...
- **WebResearchAgent** — DuckDuckGo-powered search for breaking news and company announcements
- **MarketDataAgent** — validated YFinance tool calls for prices, analyst ratings, fundamentals, and news
- **FinPulse Intelligence Team** — coordinator that routes each query to the right agent(s) and synthesises a unified answer
...
**Validation.** `models/schemas.py` defines strict Pydantic models for every tool output — non-finite prices are rejected, tickers normalised, malformed rows skipped rather than corrupting the response.

**Structured logging.** Every settings load, agent build, cache hit/miss, retry attempt, and tool execution time is logged through Python's `logging` module — zero `print()` statements.
...
**Observability.** `core/metrics.py` exposes Prometheus-format metrics at `/observability/metrics` — HTTP request latency histograms (by path/method/status), in-flight request count, agent run duration, tool execution outcomes, and cache hit/miss ratio.
...
**CI/CD.** `.github/workflows/deploy.yml` runs on every push to `main`: ruff lint + format check → pytest with coverage → Docker build + container boot smoke-test → `flyctl deploy`. Each stage gates the next.
...
`DATABASE_URL` is optional — omit it to use a local SQLite file automatically.
...
### Render.com (recommended — free tier, no credit card required)
...
3. Click **New → Blueprint**, connect your GitHub repo. Render auto-detects `render.yaml`.
...
**Note:** the free tier sleeps after 15 minutes of inactivity and takes ~30-60s to wake up on the next request — fine for a portfolio demo, not for production traffic. Render auto-deploys on every push to `main` once connected.
...
- **New data source** → add a validated tool function in `tools/`, define its Pydantic schema in `models/schemas.py`, register it on an agent in `agents/team.py`.
- **New agent** → write a `build_*_agent(settings)` factory in `agents/team.py`, add it to `build_team()`'s member list and to `core/app.py`'s `AgentOS(agents=[...])` call.
- **New metric** → add a `Counter`/`Histogram` in `core/metrics.py`, increment it where the event occurs (mirrors the pattern in `utils/cache.py` and `tools/finance_tools.py`).
